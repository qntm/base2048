...
describe('gen', () => {
  ...
  it('compresses', () => {
    expect(resolved.compressed).toEqual([
  ...
  it('does not provide enough guaranteed unbreakable characters for the whole encoding', () => {
    const needed = resolved.full.reduce((acc, repertoire) => acc + [...repertoire].length, 0)
    expect(resolved.wordBreakable.no.length < needed).toBe(true)
    expect(resolved.wordBreakable.no.length).toBe(2026)
    expect(resolved.wordBreakable.maybe.length).toBe(317)
  })
})
